...
def knn_points(query_points, support_points, k=3):

    didt_sq = torch.sum((query_points.unsqueeze(1) - support_points.unsqueeze(0))**2, dim=-1)
    _, indices = torch.topk(didt_sq, k, dim=-1, largest=False, sorted=True)
